"""
Comet Stremio-addon scraper.

Comet exposes torrent results through its standard Stremio stream API:

    /stream/movie/<imdb_id>.json
    /stream/series/<imdb_id>:<season>:<episode>.json

Cauldron consumes the torrent metadata returned by Comet and converts it
into TorrentResult objects. Comet's already-resolved playback URLs are
intentionally ignored; Cauldron handles debrid resolution itself.
"""
...
class CometScraper(Scraper):
    """Scrape torrent metadata from the configured Comet instance."""

    name = "comet"

    def __init__(self) -> None:
        self.base_url = "https://comet.forthewizards.uk"
    ...
    async def search(
        ...
        *,
        ...
        """
        Query Comet's Stremio stream endpoint.

        Comet is ID-driven, so an IMDb ID is required. The free-text query
        remains part of the Scraper interface for compatibility with the
        rest of Cauldron.
        """
        ...
        for stream in streams:
            ...
            # Comet sometimes returns informational/non-torrent streams.
            # A valid infoHash is the safest indicator that this is usable
            # torrent metadata for Cauldron.
    ...
    def _build_magnet(
        ...
        Comet supplies infoHash separately from tracker sources. We preserve
        usable tracker URLs when available, while ignoring DHT-only entries.
        """
        ...
        for source in sources:
            ...
            if source.startswith("tracker:"):
                ...
                # Ignore DHT entries and anything else that is not an
                # actual tracker URL.
                continue
